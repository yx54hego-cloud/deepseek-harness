...
flowchart LR
  ...
  pkg_modules["modules"]
  pkg_hmr["hmr"]
  pkg_mobile_access["mobile-access"]
  svc_mobileAccess["ctx.mobileAccess<br/>Encrypted mobile Session carrier"]
  ...
  pkg_mobile_access --> svc_mobileAccess
...
| `ctx.mobileAccess` | `core` | `mobile-access` | - | - | - | 为 Web bundle 的手机连接持有配对、认证、加密和窄化的 Session 请求 allowlist。 |
