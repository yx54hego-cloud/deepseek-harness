...
The dsh browser-surface bundle. [`cordis.patch.yml`](cordis.patch.yml) rides over [`dsh-base`](../base/README.md): it sets the coding persona, inserts the Web host rows (webserver, API gateway, workspace, projection cache, storage) and browser plugin roster, mounts this package's `web-runtime` glue, and starts the authenticated encrypted [`mobile-access`](../../host/mobile-access/README.md) carrier on port 6769. The ordinary `web-startup` provider parses `--host`, `--port`, repeatable `--trusted-host`, and the app's `--help`, then provides `webStartup`; the mobile listener does not change the loopback browser bind. The always-on client HMR chain remains idle until a rebuild watcher rewrites client bundles. [`dsh-headless`](../headless/README.md) is a sibling surface over the same base and mounts neither browser nor mobile carrier.
