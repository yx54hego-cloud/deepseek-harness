...
Use Expo Go or a development build on a phone connected to the same network as the Harness computer. Start the computer side with `pnpm dsh web --mobile` from source, or `dsh web --mobile` from an installed build, then scan its terminal QR.
...
- Transcript rendering is plain text. The drawer's workspace labels are derived from Session working directories; adding a workspace remains a Web-side operation. The attachment picker accepts PNG, JPEG, WebP, and GIF images only; general documents remain unsupported. Markdown, tool cards, reasoning presentation, approvals, questions, cancellation, terminals, files, Git, voice input, and collaboration remain unavailable. Their reserved layout controls are visibly disabled.
